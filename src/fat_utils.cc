...
void fatMultiply(const mat &A1, const mat &A2,
  ...
  if (A1.n_rows > 0 && A1.n_cols > 0 && A2.n_rows > 0 && A2.n_cols > 0){
    ...
    colvec y_bottom = y.subvec(x1.n_rows, y.n_rows-1).unsafe_col(0);
    fatMultiply (A2, x2, multiplier, y_bottom);
  }
  else {
    colvec y_top = y.subvec(0, x1.n_rows-1).unsafe_col(0);
    fatMultiply (A1, x1, multiplier, y_top);
  }
...
void fatMultiply(const mat &A1, const mat &A2,
  ...
  if (A1.n_rows > 0 && A1.n_cols > 0 && A2.n_rows > 0 && A2.n_cols > 0){
    ...
    fatMultiply(A2, x2, multiplier, y2);
  }
  else {
    fatMultiply(A1, x1, multiplier, y1);
  }

}

/* solve the above problem, but assume that one of the two matrices
passed is empty. */
void fatMultiply(const mat &A1,
  ...
  const rowvec temp = (A1 * x1).t();
  y1 = (temp * A1).t() + x1 * multiplier;
}
